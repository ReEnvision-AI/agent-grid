...
from agentgrid.client.routing.sequence_info import RemoteSequenceInfo
#from agentgrid.client.routing.spending_policy import NoSpendingPolicy
from agentgrid.data_structures import ModuleUID, RemoteSpanInfo, ServerState
from agentgrid.server.handler import TransformerConnectionHandler
from agentgrid.utils.dht import get_remote_module_infos
...
class SequenceManagerConfig(ClientConfig):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "agentgrid.client.routing.SequenceManagerConfig has been moved to agentgrid.ClientConfig. ",
            DeprecationWarning,
            stacklevel=2,
        )
...
class RemoteSequenceManager:
    ...
    def get_request_metadata(
        ...
        :param args_structure: the structure of flattened tensors from pack_args_kwargs in agentgrid.utils.packaging
        :param args: request-specific inputs, typically block uids and input tensors
        :param kwargs: additional request context, such as remote peer ID
        :returns: msgpack-serialized metadata dict that will be passed alongside a given request
        """
        return dict(
            points=0,#self.policy.get_points(protocol, *args, **kwargs),
...
class MissingBlocksError(RuntimeError):
    def __init__(self, block_indices: Union[int, Sequence[int]]):
        super().__init__(
            ...
            f"You can check the public swarm's state at https://www.sociallyshaped.net/health "
        )
