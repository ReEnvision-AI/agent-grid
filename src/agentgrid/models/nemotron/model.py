...
from agentgrid.models.nemotron.modeling_decilm import DeciLMForCausalLM, DeciLMModel, DeciLMPreTrainedModel

from hivemind import DHT, get_logger

from agentgrid.models.nemotron.transformers_4_44_2__cache_utils import Cache
from agentgrid.models.nemotron.variable_cache import VariableCache

logger = get_logger(__name__)



class DistributedNemotronModel(FromPretrainedMixin, PTuneMixin, DeciLMModel):
    _keys_to_ignore_on_load_missing = PTuneMixin._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = [r"^model\.layers\."]

    config_class = DistributedNemotronConfig

    def __init__(self, config: DistributedNemotronConfig, *, dht: Optional[DHT] = None):
...
class DistributedNemotronForCausalLM(FromPretrainedMixin, RemoteGenerationMixin, DeciLMForCausalLM):
    _keys_to_ignore_on_load_missing = DistributedNemotronModel._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedNemotronModel._keys_to_ignore_on_load_unexpected

    config_class = DistributedNemotronConfig

    def __init__(self, config: DistributedNemotronConfig):
        DeciLMPreTrainedModel.__init__(self, config)
        self.model = DistributedNemotronModel(config)
    ...
    @property
    def transformer(self) -> DistributedNemotronModel:  # For compatibility with RemoteGenerationMixin
        return self.model