...
_CLASS_MAPPING = {}  # Populated by agentgrid.models.* subpackages with register_model_classes()
...
class _AutoDistributedBase:
    ...
    def from_pretrained(cls, model_name_or_path: Union[str, os.PathLike, None], *args, **kwargs) -> PretrainedConfig:
        ...
        if proper_cls is None:
            raise ValueError(f"Agent Grid does not have {cls.__name__} for model type {config.model_type}")
...
class DefaultRevisionMixin:
    """
    Agent Grid only supports Falcon loaded in the new in-library format (transformers.FalconModel).
